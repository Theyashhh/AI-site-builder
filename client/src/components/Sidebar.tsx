...
interface SidebarProps{
    ...
    setIsGenerating: (isGenrating: boolean)=> void;
}
...
try {
  ...
  return (
    <div
  className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800
...
<div className='flex items-center gap-2'>
  <textarea
    ...
    className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700
     focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all'
    disabled={isGenerating}
  />
  <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600
   hover:from-indigo-600 to-indigo-700 text-white transition-colors disabled:opacity-60'>
    {isGenerating ? (
